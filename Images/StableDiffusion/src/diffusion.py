...
class TimeEmbedding(nn.Module):
    def __init__(self, n_embed):
        super().__init__()
        self.linear_1 = nn.Linear(n_embed, 4) 
...
class UNET_AttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, context: torch.Tensor) -> torch.Tensor:
        ...
        x, gate = self.linear_geglu_1(x).shunk(2, dim=-1)
        x = x * F.gelu(gate)
...
class UNET(nn.Module):
    def __init__(self):
        ...
        self.encoders = nn.Module([
            SwitchSequential(nn.Conv2d(4, 320, kernel_size=3, stride=1, padding=1)),        # (Batch_Size, 4, Height/8, Width/8)  ->  (Batch_Size, 320, Height/8, Width/8)
        ...
        self.decoder = nn.Module([
            ...
            SwitchSequential(UNET_ResidualBlock(2560, 1280), UpSample(1280)),               # (Batch_Size, 2560, Height/64, Width/64)  ->  (Batch_Size, 1280, Height/32, Width/32)
            SwitchSequential(UNET_ResidualBlock(2560, 1280), UNET_AttentionBlock(8, 160)),
            SwitchSequential(UNET_ResidualBlock(1920, 1280), UNET_AttentionBlock(8, 160), UpSample(1280)), 
            SwitchSequential(UNET_ResidualBlock(1920, 640), UNET_AttentionBlock(8, 80)),  # (Batch_Size, 1920, Height/32, Width/32)  ->  (Batch_Size, 1280, Height/16, Width/16)
            SwitchSequential(UNET_ResidualBlock(1290, 640), UNET_AttentionBlock(8, 80)),
...
class Diffusion(nn.Module):
    ...
    def forward(self, latent: torch.Tensor, context:torch.Tensor, time: torch.Tensor):
        ...
        # context: (Batch_Size, Seq_Length, Dim)
        # time: (1, 320)
        ...
        output = self.final(output)                 # (Batch_Size, 320, Height / 8, Width / 8)  ->  (Batch_Size, 4, Height, Width)
