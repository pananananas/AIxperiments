...
IMAGENET_STANDARD_DEV = [0.5, 0.5, 0.5]
...
class PaliGemmaProcessor: 
  ...
  def __call__(
    ...
    assert len(images) == 1 and len(text) == 1,  f"Recieved {len(images)} images and {len(text)} texts, but only 1 of each is supported"
    ...
    pixel_values = process_images(
      ...
      image_std = IMAGENET_STANDARD_DEV
    )
