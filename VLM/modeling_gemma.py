...
class GemmaRMSNorm(nn.Module):
  ...
  def _norm(self, x):
    return x * torch.rsqrt(x.pow(2).mean(-1, keepdim=True) + self.eps)  # 1 / sqrt(...), added eps so we dont div by 0
...
class GemmaRotaryEmbedding(nn.Module):
  def __init__(self, dim, max_position_embedding=2048, base=1000, device=None):
    super().__init__()
...
class GemmaAttention(nn.Module):
  def __init__(self, config: GemmaConfig, layer_idx: Optional[int] = None):
    ...
    self.num_key_value_gropus = self.num_heads // self.num_key_value_heads
...
class PaliGemmaForConditionalGeneration(nn.Module):
  ...
  def _merge_input_ids_with_image_features(
    ...
    scaled_image_features = image_features / (self.config.hidden_size**0.5)                       # Shape: [Batch_Size, Seq_Len, Hidden_Size]
  ...
  def forwars(
    ...
    # Get the input embeddings from language model
    inputs_embeds = self.language_model.get_input_embeddings()(input_ids)
    # Get the vision features
    selected_image_feature = self.vision_tower(pixel_values.to(inputs_embeds.dtype))
    # Project the vision features to the model dimension
